...
public class ManhattanExperiment
		extends AbstractExperiment {

	public ManhattanExperiment(SimulationParameters parameters) {
		super(parameters);
	}
	...
	public ExperimentData getGatheredData(TrafficSimWorld world) {
		...
		float carsPerMinute = vertexEntity.getComponent(IntersectionThroughputComponent.class).getTotalCarsPassed();
		...
		dataLists.add(new DataList("Average Travel time:", world.getDataGatherer().getAverageTimeTravelled()));
